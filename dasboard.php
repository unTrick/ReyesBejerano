						...
						while($rowService=mysqli_fetch_array($queryService)){ ?>
							...
							<div class="question" style="margin-bottom: 20px;" id="appointment-confirmation">
								...
								<form method="POST" action="yes.php">
									...
									<input type="hidden" name="payment" id="paymentStatus" value="settled" >
									<input type="hidden" name="reference_number" value="00000000" >
									<div class="control-group">
										...
										<div class="controls">
											<select name="payment_option" id="payment-option">
												...
												<option value="QR">QR Payment thru GCASH</option>
											</select>
										</div>
									</div>
...
<script>
	$(document).ready(function(){
        ...
        $("#payment-option").on('change', function() {
          var paymentSelected = $(this).find(":selected").val();
          if(paymentSelected == "Cash"){
              $("#paymentStatus").val("settled");
          }
          else{
              $("#paymentStatus").val("unsettled");
          }
        });
